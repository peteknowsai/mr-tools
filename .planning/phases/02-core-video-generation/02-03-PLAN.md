...
3. Output handling already works for both - the result dict has same structure:
   - {"status": "complete", "job_id": "video_...", "filepath": "..."} on success
   - {"status": "error", "job_id": "video_...", "error": "..."} on failure
...
**JSON output format:**
\`\`\`json
{"status": "complete", "job_id": "video_20260119_123045", "filepath": "/path/to/video.mp4", "elapsed_seconds": 45}
\`\`\`

**Error format:**
\`\`\`json
{"status": "error", "job_id": "video_20260119_123045", "error": "Rate limit exceeded"}
\`\`\`

**Timeout format:**
\`\`\`json
{"status": "timeout", "job_id": "video_20260119_123045", "error": "Generation timed out after 600 seconds"}
\`\`\`

The `job_id` is always returned and can be used for status tracking or manual retry.
```
...
5. Update the Confirmed Capabilities table if present.
  ...
  <done>CLAUDE.md updated with video generation documentation including job_id in JSON output</done>
</task>
...
- CLAUDE.md shows job_id in JSON output format
...
JSON output includes job_id for status tracking.
</success_criteria>
